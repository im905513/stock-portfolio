...
| `GET /api/ai/screen?market=TW&max_pe=15&min_yield=5` | 篩選**已追蹤**候選股 (watch_status=active) | 「在我追的股票裡找 PE < 15 殖利率 > 5%」 |
| `GET /api/ai/discover?industry=金融&min_score=0.6` | **全市場挖掘** DCA 候選 (FinMind 全台股) | 「幫我找 5 檔適合定期定額的新股票」|
| `POST /api/ai/watchlist/add` body `{"symbol":"2303"}` | 把新股加入 watchlist (不買，只追蹤) | discover 之後加追蹤 |
...
```
參數：`market` / `style` (dca|thematic|trade) / `sector` / `max_pe` / `min_yield` / `held_only`。

⚠️ **重要限制**：`/screen` **只看 watch_status='active'** 的股票（DB 已追蹤的），不會挖到 DB 以外的新標的。
回傳 `candidates[]`，已排序（未持有優先、PE 低優先）。

### 3.3b 「幫我找 DB 以外、適合定期定額的新股票」

```bash
GET /api/ai/discover?market=TW&min_score=0.6&limit=10
GET /api/ai/discover?industry=金融&min_score=0.55
GET /api/ai/discover?industry=半導體&pool=80
```
參數：
- `market` — 目前只支援 TW (FinMind)
- `min_score` — DCA 適合度門檻 0–1（建議 0.5–0.65）
- `limit` — 回傳幾檔（最多 50）
- `pool` — 從多大候選池評分（預設 40，越大越慢）
- `industry` — 子字串比對台股產業；留空 = 預設 DCA 友善類別（金融/食品/半導體/電信/公用/塑膠/鋼鐵）
- `exclude_held` — 預設 true，自動排除你已持有的

回傳每檔含 `symbol`、`name`、`industry`、`per`、`pbr`、`dividend_yield`、`score` (0–1)、`reasons[]`、`in_watchlist`。
**評分權重**：殖利率 0.35 / PE 合理區 0.25 / PBR 0.20 / 營收揭露 0.20。
**首次呼叫 ~15–30s**（FinMind 限流），1 小時內同條件秒回快取。
...
{ "symbol": "2303", "name": "聯電", "sector": "半導體" }   # name/sector 可省略，會從 FinMind 補
```

加入後該檔變 `watch_status='watchlist'` — 不會出現在持股報表，但 `/api/ai/stock/{symbol}` 可正常查它的估值/法人/營收，方便後續深度分析。確認真的要買時再 `POST /api/trades`。
...
### Q: 「幫我找 PE 低、殖利率高的金融股」(在我已追蹤的範圍內)
1. `GET /api/ai/screen?sector=金融&max_pe=15&min_yield=4.5`
2. 排序結果給使用者
3. 對前 3 名各打 `GET /api/ai/stock/{symbol}` 拉詳細資料做比較表

### Q: 「我想定期定額，幫我找幾檔可以每月買的新股票」
**這是 `/discover` 的主場景** — 不要再用 `/screen`，因為 screen 只會回傳 DB 已追蹤的 5 檔。
1. `GET /api/ai/discover?industry=金融&min_score=0.6&limit=5` — 先試金融
2. 同上再試 `industry=食品`、`industry=半導體`、`industry=電信` 三類
3. 把每類前 2-3 名整理成表，欄位：symbol / name / yield / PE / score / reasons
4. 講清楚為什麼 score 高（讀 `reasons[]`）
5. 問使用者要不要把喜歡的加入 watchlist：`POST /api/ai/watchlist/add { "symbol": "..." }`
6. 加入後**不會立刻買** — 提醒使用者後續再 `POST /api/trades` 執行
